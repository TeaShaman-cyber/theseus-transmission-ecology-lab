...
A step receipt includes the relevant state plus `source_ready`, `adapted`, `persistent`, `next_state`, exact bindings for `V`, `W_source`, `W_target`, fixture and source revision, an explicit `claimed_stages` set, and the aggregate `stage_attribution`. The canonical stage identifiers are `pre_adaptation` and `post_adaptation`. `claimed_stages` is not caller-selectable evidence: it MUST equal the set of nonidentity gates in the tested fixture (`W_source != I` -> `pre_adaptation`; `W_target != I` -> `post_adaptation`). The stage-local total masses are deterministic derivations of those three intermediate states and may be recorded for convenience, but only the bound intermediate witnesses carry attribution authority. Global receipts additionally bind `A`, `transmission_scale`, graph fixture, and node/variant dimensions.
...
- `PASS`: `claimed_stages` exactly matches the nonidentity gates; all required witnesses are present, bound and internally consistent; every claimed stage has its own valid matched identity control; each immediately-after-stage witness differs from its control counterpart at canonical precision; and the aggregate verdict is the conjunction of all claimed per-stage verdicts. A two-stage fixture therefore cannot `PASS` with only one stage control.
...
- `FAIL`: contradictory state, shape mismatch, non-finite/negative values, binding mismatch, `claimed_stages` mismatch, or a supplied control whose reference, shape, nonintervened fields, or declared intervention are invalid/mismatched.
